...
def load_data(
    path: str, k: int = 256, splits: tuple[float] = (1.0,), tq: bool = False
    ...
    with open(path, mode="r") as file:
        tracks = [
            np.array(
                [min(int(num), util.DIM - 1) for num in line.split()], dtype=np.int16
            )
            for line in (tqdm(file) if tq else file)
        ]
        windowed = np.concatenate(
            [
                np.lib.stride_tricks.sliding_window_view(track, k + 1, axis=0)
                for track in tracks
            ],
            axis=0,
        )
